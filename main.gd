...
func _ready() -> void:
	...
	player_panel_pos = $MarkerPanel.get_position()
	new_game()
			...
			if event.position.x < board_size:
				...
				if grid_data[grid_pos.y][grid_pos.x] == 0:
					...
					if check_win() != 0:
						...
						$GameOverMenu.show()
						if winner == 1:
							$GameOverMenu.get_node("ResultLabel").text = "Circles wins!"
						elif winner == -1:
							$GameOverMenu.get_node("ResultLabel").text = "Crosses wins!"
...
func check_win():
	#add up the markers in each row and diagonal
...
#Recieves the emission from game_over_menu
